<?php
	...
	$sql = "SELECT DISTINCT
			...
			FROM
				...
				LEFT JOIN soportes ON resultadoauditoria.codigoNC = soportes.codigoNC AND resultadoauditoria.Criterio = soportes.Criterio AND resultadoauditoria.idObra = soportes.idObra
			WHERE
	...
	while ($row = mysqli_fetch_assoc($result))
	{
		...
		$renglones = ceil(strlen($row['Descripcion'])/60);
		...
		$pdf->Cell(53, (4 * $renglones), $row['Observaciones'] ,1,0,'L');
